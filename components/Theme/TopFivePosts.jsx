...
const TopFivePosts = ({ isSubCategory }) => {
    ...
    return (
        <div className='category-realtime-top5-posts-div'>
            ...
            .map((post) => (
                <Link onClick={() => {gtag.event({ action: "Go to Category Top Article", category: "Paging", label: "category page" })}} href={`/${post.Post.Category.domain}/${post.Post.SubCategory.domain}/${post.PostId}`} key={`${post.Post.title}-link`}><a>
